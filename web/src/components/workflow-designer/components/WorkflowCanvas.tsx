...
function WorkflowCanvas(props: WorkflowCanvasProps) {
  ...
  // Architecture icons helper used by node rendering in architecture mode
  const getArchitectureIconSvg = useCallback((type: string, size: number, color: string) => {
    const iconMap: Record<string, React.ReactElement> = {
      server: <Server width={size} height={size} color={color} />,
      database: <Database width={size} height={size} color={color} />,
      globe: <Globe width={size} height={size} color={color} />,
      shield: <Shield width={size} height={size} color={color} />,
      monitor: <Monitor width={size} height={size} color={color} />,
      cloud: <Cloud width={size} height={size} color={color} />,
      branch: <GitBranch width={size} height={size} color={color} />,
      package: <Package width={size} height={size} color={color} />,
      users: <Users width={size} height={size} color={color} />,
      file: <FileText width={size} height={size} color={color} />,
      box: <Box width={size} height={size} color={color} />,
    }
    const el = iconMap[type] ?? <Box width={size} height={size} color={color} />
  ...
  designerMode: workflowContextState.designerMode as DesignerMode | undefined,
    getConfigurableDimensions: (nodeData: unknown) => {
      // nodeData is WorkflowNode at runtime
  ...
  // Invoke node rendering after helpers are defined above
  useNodeRendering({
    svgRef,
    nodes,
    connections,
  ...
  // Connections-only rendering handled by hook
  ...
  designerMode: workflowContextState.designerMode as DesignerMode | undefined,
    nodeMap,
    onConnectionClick: (c) => onConnectionClick(c),
    createFilledPolygonFromPath,
  })

  // (migrated) root SVG events now handled by useCanvasEvents

  // Grid effect isolated to size + showGrid only (prevents re-render cascade)
