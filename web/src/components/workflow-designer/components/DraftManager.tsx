  ...
  return (
    ...
    >
      <div className="draft-item-main">
        ...
        <div>
          <h4 className="draft-item-name" title={draft.name}>
            {draft.name}
          </h4>
          <p
            className="draft-item-date"
            title={new Date(draft.metadata.updatedAt).toLocaleString()}
          >
      ...
      <div className="draft-item-meta-row">
        <span className="draft-version" title={`Version ${draft.metadata.version}`}>
...
export default function DraftManager({ isOpen, onClose }: Readonly<DraftManagerProps>) {
  const { state, saveDraft, loadDraft, deleteDraft, listDrafts, getStorageStats } =
    useWorkflowContext();
  ...
  const handleSaveDraft = useCallback(async () => {
    ...
    try {
      ...
      const draftId =
        ...
        `draft-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
      saveDraft(draftId, newDraftName.trim());
      if (!state.currentDraftId) {
        console.warn('Draft created & set as current');
      } else {
        console.warn('Draft updated (version increment)');
      }
      refreshData();
    } catch (error) {
  ...
  const commitRename = useCallback(
    async (draftId: string) => {
      ...
      if (drafts.find((d) => d.id === draftId)?.name === name) {
        setIsEditingName(false);
        return;
      }
      setIsLoading(true);
  ...
  const handleLoadDraft = useCallback(
    async (draftId: string) => {
      ...
      try {
        ...
        if (success) {
          onClose();
          console.warn('Draft loaded successfully');
        } else {
  ...
  const handleDeleteDraft = useCallback(
    async (draftId: string, draftName: string) => {
      ...
      try {
        ...
        console.warn('Draft deleted successfully');
      } catch (error) {
  ...
  const filteredDrafts = useMemo(() => {
    ...
    base.sort((a, b) => {
      ...
      switch (sort.key) {
        ...
        case 'updated':
          return (a.metadata.updatedAt - b.metadata.updatedAt) * dirMul;
        case 'created':
          return (a.metadata.createdAt - b.metadata.createdAt) * dirMul;
        case 'version': {
  ...
  const handleKeyNav = useCallback(
    (e: React.KeyboardEvent) => {
      ...
      if (['ArrowDown', 'ArrowUp', 'Enter', 'Delete', 'Backspace'].includes(e.key)) {
        e.preventDefault();
      }
      const currentIndex = selectedDraft
        ? filteredDrafts.findIndex((d) => d.id === selectedDraft)
        : -1;
      if (e.key === 'ArrowDown') {
        ...
        if (target) {
          handleDeleteDraft(selectedDraft, target.name);
        }
      }
    },
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      ariaLabelledBy="draft-manager-title"
      className="draft-manager-modal"
      withHeader={false}
      closeOnBackdrop={false}
    >
      <div onKeyDown={handleKeyNav} tabIndex={0} ref={listContainerRef}>
        {/* Header */}
        <div className="draft-manager-header">
          <div className="draft-manager-title">
            <FileText size={24} className="draft-manager-icon" />
            <div>
              <h2 id="draft-manager-title">Draft Manager</h2>
          ...
          <div className="draft-manager-actions">
            ...
            <button
              ...
              className={`draft-action-btn ${showSettings ? 'active' : ''}`}
              title="Settings"
            >
              <Settings size={16} />
            </button>
            <button onClick={onClose} className="draft-action-btn" title="Close">
              <X size={16} />
            </button>
          </div>
        </div>
          ...
          <div className="draft-list-panel">
            ...
            <div className="draft-search-section">
              ...
              <div className="draft-filter-tabs sort-bar">
                <button
                  type="button"
                  className={`draft-filter-tab ${sort.key === 'updated' ? 'active' : ''}`}
                ...
                <button
                  type="button"
                  className={`draft-filter-tab ${sort.key === 'name' ? 'active' : ''}`}
                    ...
                    <>
                      ...
                      <div className="draft-info-cards">
                        ...
                        <div className="draft-info-card">
                          ...
                          <div className="draft-info-value">
                            {formatDate(draft.metadata.updatedAt)}
                          </div>
                        </div>
                        <div className="draft-info-card">
                        ...
                        <div className="draft-info-card">
                          ...
                          <div className="draft-info-value">Draft</div>
                        </div>
                      </div>
                    </>
                  );
                  ...
                  <div className="draft-storage-stats">
                    ...
                    <div className="draft-stats-grid">
                      ...
                      <div className="draft-stat-item">
                        ...
                        <span className="draft-stat-value">
                          {storageStats.compressedCount} ({storageStats.compressionRate}%)
                        </span>
                      </div>
                      <div className="draft-stat-item">
